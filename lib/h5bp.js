...
h5bp.createServer = function(options, callback) {
    ...
    if (/(express|connect)/.test(options.server)) {
        ...
        express = require(options.server)
        , connect = require('connect')
        , app = express();

        
        if (true === options.logger || 'object' == typeof options.logger) {
            middlewares.unshift(express.logger(options.logger));
        }
        if (options.root) {
            middlewares.push(express.static(options.root));
            middlewares.push(express.favicon(options.root));
        }
...
var mime = {
    lookup: function (path) {
        ...
        return h5bp.mimeTypes[ext] || 'text/html';
    }
};
